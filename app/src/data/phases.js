...
export const VIZIER_PHASES = [
  {
    ...
    title: 'Opening',
    turnRange: 'T1–T20',
    summary: 'Settle capital, explore, expand to City 2, claim Pantheon.',
    items: [
      {
        id: 'assess_start',
      ...
      {
        id: 'build_scout',
        text: 'First build: Warrior (default), Slinger, or Scout — map dependent',
        detail: 'Warrior = safe default; explores at 2 movement AND fights Barbarians. Slinger = explore + Archery Eureka from first kill. Scout = best on large/safe maps where extra movement compounds. Warriors/Slingers are not a fallback — they often outperform Scout at higher difficulties.',
        priority: 'E',
      },
      {
        id: 'move_warrior',
        text: 'Move starting Warrior in opposite direction from your build unit',
        priority: 'H',
      },
      {
        id: 'research_pottery',
      ...
      {
        id: 'build_monument',
        text: 'Build Monument after first unit (unless active Barbarian threat)',
    ...
    checkpoints: [
      {
        ...
        question: 'Did you find a Natural Wonder?',
        type: 'boolean',
...
export const SULTAN_PHASES = [
  {
    ...
    items: [
      ...
      {
        id: 'build_scout',
        text: 'Second build: Slinger or Scout — explore while Warriors guard',
        detail: 'Slinger gets Archery Eureka from first kill (unlocks Archers early — useful for Sultan). Scout covers more ground per turn. Either works; Slinger has slightly more utility at higher difficulties.',
        priority: 'H',
      },
      {
        id: 'research_pottery',
...
export const SITUATIONS = [
  {
    ...
    phase: 'any',
    severity: 'critical',
    summary: 'Your Mamluk window is at risk.',
    actions: [
      'Check whether Horses are outside your borders but close enough to buy or culturally claim.',
      'Trade for Horses from any civ that has surplus.',
      'Plan a limited war or forward city if a nearby Horse tile is reachable.',
      'If no source is realistic, pivot military to Crossbowmen + Catapults and treat Domination as weaker.',
    ],
    avoid: 'Do not plan a Mamluk timing push without a confirmed Horse source.',
    guideRefs: ['situations', 'midGame', 'techCivics'],
  },
  {
    ...
    severity: 'high',
    summary: 'Protect your core religion before the pressure compounds.',
    actions: [
      'Position an Apostle in the threatened city, ideally with Debater.',
      'Attack incoming Missionaries/Apostles with Theological Combat.',
      'Faith-buy a Missionary to restore pressure in the converted city.',
      'If the same civ keeps sending units, consider Holy War.',
    ],
    avoid: 'Do not let your core cities quietly flip; one converted city makes the next one easier.',
    guideRefs: ['situations', 'overview', 'victoryPaths'],
  },
  {
    ...
    phase: 'religion_window',
    severity: 'medium',
    summary: 'The plan is slower, not dead.',
    actions: [
      'Take Feed the World plus Church Property if available.',
      'Build Madrasas with Production instead of assuming Faith purchases.',
      'Audit Gold income early because missing Tithe hurts scaling.',
      'Lean harder on Campuses and Mamluk conquest to compensate.',
    ],
    avoid: 'Do not restart the game just because the ideal belief pair is gone.',
    guideRefs: ['overview', 'earlyGame', 'victoryPaths'],
  },
  {
    id: 'hemmed_in',
    ...
    severity: 'high',
    summary: 'You need land before the mid-game engine stalls.',
    actions: [
      'Settle the best available site even if it is not perfect.',
      'Prioritise Campus adjacency over Holy Site adjacency when compromising.',
      'With Sultan, evaluate an early Warrior + Catapult war against the weakest blocker.',
      'With Vizier, prepare to take a better city during the Mamluk window.',
    ],
    avoid: 'Do not wait for perfect land while the AI claims every workable city site.',
    guideRefs: ['situations', 'cityPlacement', 'midGame'],
  },
  {
    id: 'low_faith',
    ...
    phase: 'mamluk_window',
    severity: 'medium',
    summary: 'Your spending is outpacing the engine.',
    actions: [
      'Check that Worship Buildings are deployed in all Temple cities.',
      'Slot Triangular Trade and Scripture if they fit your current government.',
      'Add Trade Routes before buying more religious units.',
      'Delay Missionary spam until Faith income catches up.',
    ],
    avoid: 'Do not keep buying Missionaries if it prevents Madrasas or key Theocracy purchases.',
    guideRefs: ['situations', 'policyCards', 'midGame'],
  },
  {
    ...
    phase: 'victory_push',
    severity: 'high',
    summary: 'Recover the Campus/Madrasa engine before committing to Science Victory.',
    actions: [
      'Put a Madrasa in every core city, faith-purchasing if Jesuit Education is active.',
      'Slot Natural Philosophy or Rationalism depending on era and requirements.',
      'Build or buy Research Labs as soon as available.',
      'Target Great Scientists and Bologna suzerainty if present.',
    ],
    avoid: 'Do not keep splitting between Science, Religion, and Domination if you are already behind.',
    guideRefs: ['situations', 'victoryPaths', 'greatPeople'],
  },
  {
    ...
    phase: 'mamluk_window',
    severity: 'critical',
    summary: 'Stabilise before the conquest snowball reverses.',
    actions: [
      'Assign a Governor immediately, usually Victor for contested territory.',
      'Convert the city with an Apostle as soon as possible.',
      'Repair Monument and core infrastructure before pushing deeper.',
      'Make peace if loyalty pressure will flip the city before reinforcements arrive.',
    ],
    avoid: 'Do not keep conquering while your newest city is about to revolt.',
    guideRefs: ['situations', 'midGame', 'victoryPaths'],
  },
]
